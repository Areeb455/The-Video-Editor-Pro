...
def auto_cut():
    ...
    # Sort marks by time
    marks = sorted(marks, key=lambda m: float(m.get('time', 0)))
    ...
    try:
        with open(concat_file_path, 'w') as f:
            for i in range(0, len(marks), 2):
                start = float(marks[i]['time'])
                end = float(marks[i+1]['time'])
                duration = end - start
                if duration <= 0.05:
                    continue
...
def analyze_style():
    ...
    try:
        ...
        # Get duration from format (more reliable than stream)
        ...
        # Get frame rate
        fps_str = v_stream.get('r_frame_rate', '30/1') if v_stream else '30/1'
        try:
        ...
        # Use ffprobe scene detection to estimate cuts
        # Run scene detection via FFmpeg select filter
        scene_cmd = [
            'ffprobe', '-v', 'quiet',
            '-show_frames',
            '-of', 'json',
            '-select_streams', 'v',
            '-read_intervals', '%+30',  # analyze first 30 seconds
        ...
        try:
            ...
            for fi in sample_frames:
                ...
                # Reshape and compute mean color of quadrants
                h, w, _ = small.shape
                for qr in range(2):
                    for qc in range(2):
                        quad = small[qr*h//2:(qr+1)*h//2, qc*w//2:(qc+1)*w//2]
                        mean_bgr = quad.reshape(-1, 3).mean(axis=0)
                        b, g, r = int(mean_bgr[0]), int(mean_bgr[1]), int(mean_bgr[2])
                        dominant_colors.append((r, g, b))
            cap.release()
            
            # Get top 6 unique-ish colors
            seen = set()
            ...
            print(f"OpenCV palette extraction error: {cv_err}")
        
        if not palette:
            palette = ['#e74c3c', '#3498db', '#2c3e50', '#ecf0f1', '#f39c12', '#2ecc71']
        
        # Estimate color temperature from palette
        avg_r = avg_b = 0
        try:
            rgb_vals = []
            for hex_color in palette:
                h = hex_color.lstrip('#')
                rgb_vals.append((int(h[0:2], 16), int(h[2:4], 16), int(h[4:6], 16)))
        ...
        # Factors: resolution quality, has audio, pacing, duration
        score = 5.0
        if width >= 1920:
            score += 1.5
        elif width >= 1280:
